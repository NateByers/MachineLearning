...
*Please use R to manually compute the weight vector* **w** = [w<sub>0</sub>, w<sub>1</sub>, w<sub>2</sub>]^T^ *for linear regression of N input data points* $(x_{i}, y_{i})$ *that minimizes the sum of squared error* $(\mathbf{w}^{T}\mathbf{x}_{i} – y_{i})^{2}$. *The input data is* 
...
*(Note that the first element in vector* **x** *is NOT* $x_{0}=1$ *in this problem), please use R to manually find a vector* **w**=[w1,w2] *with Fisher’s LDA and provide the intermediate steps (20 points).*
...
$$\mathrm{ln}\lambda =\mathrm{ln}\frac{P(y=-1)}{P(y=1)}\frac{c_{1,-1}}{c_{-1,1}}=\mathrm{ln}\frac{0.9}{0.1}\frac{1000}{1}=\mathrm{ln}(9.10498)$$
